...
class BaseModel(nn.Module):
    ...
    def _apply(self, fn):
        ...
        if isinstance(m, (Detect, Segment,RepHead,)):
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolov8n.yaml", ch=3, nc=None, verbose=True):  # model, input channels, number of classes
        ...
        # Build strides
        m = self.model[-1]  # Detect()
        if isinstance(m, (Detect, Segment, Pose, RepHead,OBB,RepPose,)):
            s = 640  # 2x min stride
            m.inplace = self.inplace
            forward = lambda x: self.forward(x)[0] if isinstance(m, (Segment, Pose, OBB,RepHead,RepPose,)) else self.forward(x)
            try:
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in (Classify, Conv, ConvTranspose, GhostConv, Bottleneck,
                 ...
                 C1, C2, C2f, RepNCSPELAN4, ADown, SPPELAN, C2fAttn, C3,
                 C3TR, C3Ghost, nn.ConvTranspose2d,  DWConvTranspose2d, C3x,
                 RepC3,RepConv,  nn.Conv2d, SPPELAN,
                  RepNCSPELAN4_SENetV2,
                 ):
        ...
        elif m in (Detect, WorldDetect, Segment, Pose, OBB, ImagePoolingAttn, RepHead,RepPose, ):
    ...
    if isinstance(model, nn.Module):  # PyTorch model
        ...
        for m in model.modules():
            if isinstance(m, (Segment,)):
                ...
                return "classify"
            elif isinstance(m, (Pose,  RepPose,)):
                return "pose"
